...
def remove_laser_points(current_stepper):
    """removes all laser_points from screen"""
    step_size = 60

    if last_laser_points is not None:

        move_stepper(current_stepper, (-1) * step_size)
        if len(laser_points) < len(last_laser_points):
            current_stepper = current_stepper - 1
            return current_stepper



def get_laser_back_on_goal_position(screen_empty, stepper_back_on_screen, laser_points):
    """moves the removed laser_points back on screen and on their
    dedicated goal_position"""
    step_size = 2
    mod_laser_points = []

    if not laser_points:
        mod_laser_points = [0,0,0,0,0]
    for i in range(len(laser_points)):
        mod_laser_points.append(laser_points[i][0])
    mod_laser_points.append(0)

    if screen_empty == True:
        log("screen is empty - now moving steppers back on screen!")

        if stepper_back_on_screen == []:
            if mod_laser_points[0] > 40 or mod_laser_points[0] == 0:
                move_stepper(0, step_size)
            if mod_laser_points[0] < 40 and mod_laser_points[0] != 0:
                stepper_back_on_screen = [1]

        if stepper_back_on_screen == [1]:
            if mod_laser_points[1] > 80 or mod_laser_points[1] == 0:
                move_stepper(1, step_size)
            if mod_laser_points[1] < 80 and mod_laser_points[1] != 0:
                stepper_back_on_screen = [1,1]

        if stepper_back_on_screen == [1,1]:
            if mod_laser_points[2] > 120 or mod_laser_points[2] == 0:
                move_stepper(2, step_size)
            if mod_laser_points[2] < 120 and mod_laser_points[2] != 0:
                stepper_back_on_screen = [1,1,1]

        if stepper_back_on_screen == [1,1,1]:
            if mod_laser_points[3] > 160 or mod_laser_points[3] == 0:
                move_stepper(3, step_size)
            if mod_laser_points[3] < 160 and mod_laser_points[3] != 0:
                stepper_back_on_screen = [1,1,1,1]

        if stepper_back_on_screen == [1,1,1,1]:
            if mod_laser_points[4] > 200 or mod_laser_points[4] == 0:
                move_stepper(4, step_size)
            if mod_laser_points[4] < 200 and mod_laser_points[4] != 0:
                stepper_back_on_screen = [1,1,1,1,1]

        if stepper_back_on_screen == [1,1,1,1,1]:
            return True
...
while True:
    ...
    # remove all laser points from screen
    if screen_empty is not True:
        current_stepper = remove_laser_points(actual_stepper)
        if current_stepper is not None:
            actual_stepper = current_stepper
        if current_stepper == -1:
            log("screen is finaly empty")
            screen_empty = True


    # move removed laser_points back on screen:
    if screen_empty is True:
        get_laser_back_on_goal_position(screen_empty, stepper_back_on_screen, laser_points)
        if get_laser_back_on_goal_position(screen_empty,stepper_back_on_screen,laser_points) == True:
            log("I'm done here - have fun!")
            break

...
def match_laser_to_stepper(matched_list):
    ...
    if last_laser_points is not None:
        if len(last_laser_points) > len(laser_points):
            ...
            # random_stepper_direction -1 *(-1) makes 0 to 1 and 1 to 0 which is backwards
            ...
            if matched_list[current_stepper] == (0,0):
                ...
                # else: Movement is found, store it in the matched_list at index "current_stepper"
                    ...
                    log("inserted coordinates in matched_list, switching to next stepper")
...
def get_laser_on_position(matched_list):
    ...
    pixel_to_steps_coefficient = 0.6
    ...
    for i in range(0, len(matched_list)):
        way_to_go_in_steps[i] = int(abs((goal_position[i] - matched_list[i][0]) * pixel_to_steps_coefficient))
    ...
    log("directions:", direction_to_step)
...
def stabilize_laser(laser_points):
    """function to stabilize the laser on their goal position. Trying to keep them there."""
    threshold = 1
    steps = 2
    """ parallel stepping method:
    stabilization_movement = [0,0,0,0,0]
    stabilization_direction = [0,0,0,0,0]
    for i in range(0,len(laser_points)):
        if laser_points[i][0] < goal_position[i] - threshold:
            stabilization_movement[i] = steps
            stabilization_direction[i] = 1
        elif laser_points[i][0] > goal_position[i] + threshold:
            stabilization_movement[i] = steps
            stabilization_direction[i] = 0
        move_steppers(stabilization_direction, stabilization_movement)
    """
    for i in range(0,len(laser_points)):
        if laser_points[i][0] < goal_position[i] - threshold:
            move_stepper(i,1,steps)
        elif laser_points[i][0] > goal_position[i] + threshold:
            move_stepper(i,0,steps)
...
while True:
    ...
    # run main program to match laser and steppers AND stabilize them at the goal position
    log(laser_points)
    log(last_laser_points)

    # if lasers are not matched, do so...
    if not lasers_matched:
        matched_lasers = match_laser_to_stepper(matched_list)
        # if they are now matched, match_laser_to_stepper returns the list, now more "none"
        if matched_lasers is not None:
            lasers_matched = True

    # if lasers are matched and laser_position_reached is False then run "get_laser_on_position" once (!)
    ...
    # set current laser points to last laser points to allow movement tracking for "find_movement_on_screen"
